...
- **ERC-5192 / ERC-5484 (soulbound tokens)**: confirmed real Ethereum EIPs. **Critical correction: these are EVM-only.** IN$DEX's canonical chain is Solana. [SUPERSEDED 2026-07-27 — see CLAUDE.md canonical mint decision] This entry originally said INDX itself is "SPL Token-2022, per whitepaper Appendix A" and recommended an SPL Token-2022 Non-Transferable extension as the soulbound mechanism. Mainnet verification (2026-07-22) confirmed INDX is a **plain SPL Token (original Token Program)**, not Token-2022 — Appendix A has been corrected accordingly, and the founder's 2026-07-27 decision retains that mint with no Token-2022 migration. The equivalent Solana-native soulbound primitive is therefore **not** a Token-2022 extension on the INDX mint — per the sitewide pattern already in use (siindex-agent-skills.html, indx-flywheel-automation.html, siindex-dev-portal.html), Merchant Trust Credentials and other non-transferable badges are issued as their own **separate Metaplex / compressed NFT (cNFT) mints**, independent of the INDX fungible token, per whitepaper Section 8. AJ's own text anticipates this ("IN$DEX needs a chain-neutral receipt definition and must use an equivalent mechanism only where the selected chain can enforce it") — the actual Solana-native mechanism is the separate soulbound cNFT/Metaplex mint, not a Token-2022 extension on INDX.
